; (function ($) {
  ...
  $(function () {
    ...
    // Swap brand logos on theme change
    var updateBrandLogos = function() {
        var isDark = $("body").hasClass("dark-theme");
        $("img.brand-logo").each(function(){
            var $img = $(this);
            var light = $img.attr("data-light");
            var dark = $img.attr("data-dark");
            if (light && dark) {
                $img.attr({src: isDark ? dark : light});
            }
        });
    };
    updateBrandLogos();
    // observe body class changes to update logos
